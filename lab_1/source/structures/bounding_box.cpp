...
std::string BoundingBox::get_string() {
    std::string res = "" + std::to_string(x_min) + " " + std::to_string(x_max) + "  -  " +
...
void BoundingBox::plotting_sanity_check() {
    ...
    if (x_size < 1e-10 && y_size > 1e-10) {
        // fix x size
        // std::cout << "BoundingBox.sanity_check: inflating bounding box in x direction." <<
        // std::endl;
    ...
    if (y_size < 1e-10 && x_size > 1e-10) {
        // fix y size
        // std::cout << "BoundingBox.sanity_check: inflating bounding box in y direction." <<
        // std::endl;
    ...
    if (x_size < 1e-10 && y_size < 1e-10) {
        throw std::invalid_argument(
            "x and y size of bounding box are 0, thus can not be fixed by the sanity check.");
    }
}
...
BoundingBox BoundingBox::get_scaled(std::uint32_t scaling_factor) {
    ...
    return BoundingBox(x_middle - x_size * scaling_factor, x_middle + x_size * scaling_factor,
                       y_middle - y_size * scaling_factor, y_middle + y_size * scaling_factor);
}

bool BoundingBox::contains(const Vector2d<double>& pos) const noexcept {
    if (x_min > pos[0] || x_max < pos[0] || y_min > pos[1] || y_max < pos[1]) {
        return false;
    }
    return true;
}
BoundingBox BoundingBox::get_quadrant(std::uint8_t quadrant) const {
    auto x_mid = (x_max + x_min) /2;
    auto y_mid = (y_max + y_min) /2;
    switch (quadrant) {
        case 0:
        ...
        default:
            throw std::out_of_range{"Range 0-3 allowed, got " + quadrant};
    }
}